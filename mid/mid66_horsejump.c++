...
{
	int						minPath = row * row + 1;
	queue<pair<int, int>>	q;
	pair<int, int>			curr;
	vector<vector<int>>		startPath(row + 1, vector<int>(row + 1));
	...
	while (!q.empty())
	{
		...
		for (auto jumpPoint : jump)
		{
			...
			if (isValid(x_jump, y_jump, map) && pathToEnd[x_jump][y_jump] != 0)
				minPath = min(minPath, startPath[curr.first][curr.second] + pathToEnd[x_jump][y_jump] - 1);
		}
...
int	bfs(vector<string> &map)
{
	pair<int, int>			curr;
	queue<pair<int, int>>	q;
	vector<vector<int>>		pathToEnd(row + 1, vector<int>(row + 1));

	q.push(make_pair(row, row));
	pathToEnd[row][row] = 1;
	while (!q.empty())
	{
		curr = q.front();
